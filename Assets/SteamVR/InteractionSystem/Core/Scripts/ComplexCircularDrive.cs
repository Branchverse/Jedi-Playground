...
namespace Valve.VR.InteractionSystem
{
	...
	public class ComplexCircularDrive : MonoBehaviour
	{
		public enum Axis_t
		{
			XAxis,
        ...
        private void Start()
		{
			...
			if (transform.parent)
			{
				Debug.Log("Adapting to parent");
				worldPlaneNormal = transform.parent.localToWorldMatrix.MultiplyVector( worldPlaneNormal ).normalized;
				localPlaneNormal = transform.parent.localToWorldMatrix.MultiplyVector( worldPlaneNormal ).normalized;
			}
		...
		private void UpdateGameObject()
		{
			...
			{
                if (!driving ){
                    transform.RotateAround(transform.position, transform.forward, startAngle);
                    lastAngle = startAngle;
                }
				Debug.Log(localPlaneNormal);
                // LOOK AT ME STEAMVR....I AM THE CAPTAIN NOW! Fuck you and your garbage rotation system
				// transform.localRotation = start * Quaternion.AngleAxis( outAngle, localPlaneNormal );
                transform.RotateAround(transform.position, transform.forward, outAngle - lastAngle);
                lastAngle = outAngle;           
			}
